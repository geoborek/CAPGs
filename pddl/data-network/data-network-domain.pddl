...
(:functions
    (total-cost) - number
    (cost-process ?sc - script ?s - server) - number
    (base-process ?sc - script ?s - server) - number
...
(:action process
    :parameters (?sc - script ?s - server ?in1 ?in2 ?out - data ?size ?limit ?before ?after - numbers)
    :precondition
    (and
    ...
    (and
        ...
        (not (usage ?s ?before))
        (usage ?s ?after)
        (increase (total-cost) (cost-process ?sc ?s))
    )
)
